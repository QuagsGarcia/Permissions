...
Citizen.CreateThread(function()
    while true do
    	for _, user in ipairs(GetPlayers()) do
	    	local discordId = nil
			...
			if discordId then
				...
				if member.code == 200 then
		            ...
		            if nickname == nil then
			        	...
			        	local member = DiscordRequest("GET", endpoint, {})
			            local data = json.decode(member.data)
			            discordData[user] = data.username;
		            else
						discordData[user] = nickname;
		            end
				else
		            discordData[user] = GetDiscordName(user);
				end
			else
				discordData[user] = GetPlayerName(user);
			end
			SetResourceKvp("PlayerNickName:"..user, discordData[tostring(user)])
		end
        	Citizen.Wait(15000)
    end
end)

lib.callback.register('DiscordAPI:getNick', function(a, srcing)
    return GetResourceKvpString("PlayerNickName:"..srcing)
end)
...
function IsDiscordEmailVerified(user) 
    ...
    if discordId then 
        ...
        if member.code == 200 then
        	...
        	print("[Quags_Perms] ERROR: Code 200 was not reached. Error provided: " .. member.data)
        end
    end
    return isVerified;
end
...
function GetGuildIcon()
	...
	if guild.code == 200 then
		...
		print("[Quags_Perms] An error occured, please check your config and ensure everything is correct. Error: "..(guild.data or guild.code)) 
	end
	return nil;
end
...
function GetDiscordAvatar(user) 
	...
	if discordId then 
		...
		print("[Quags_Perms] ERROR: Discord ID was not found...")
	end
    return imgURL;
end
...
function GetDiscordRoles(user)
	...
	if discordId then
		...
		if member.code == 200 then
            ...
            repeat
            triedGettingRoles = triedGettingRoles + 1
            local endpoint = ("guilds/%s/members/%s"):format(Config.Guild_ID, discordId)
			local member = DiscordRequest("GET", endpoint, {})
            Citizen.Wait(100)
            until(member.code == 200 or triedGettingRoles == 5)
            if member.code == 200 then
               local data = json.decode(member.data)
			local roles = data.roles
			local found = true
			return roles 
              ...
              print("[Quags_Perms] ERROR: Code 200 was not reached... Returning false. [Member Data NOT FOUND] | Tried 5 times.")
			return false  
            end
		end
	else
		print("[Quags_Perms] ERROR: Discord was not connected to user's Fivem account...")
		return false
	end
	return false
end
...
function GetDiscordNickname(user)
	...
	if discordId then
		return discordData[tostring(user)]
	else
...
Citizen.CreateThread(function()
	...
	if guild.code == 200 then
		local data = json.decode(guild.data)
		print("[Quags_Perms] Permission system guild set to: "..data.name.." ("..data.id..")")
	else
		print("[Quags_Perms] An error occured, please check your config and ensure everything is correct. Error: "..(guild.data or guild.code)) 
	end
end)